...
def get_candles(symbol, interval, limit=200):
    """Obtiene velas de Binance TESTNET usando python-binance OFICIAL"""
    try:
        # Crear cliente TESTNET - sin bloqueos geográficos
...
def get_price(symbol):
    """Obtiene precio actual de Binance TESTNET"""
    try:
        # Crear cliente TESTNET - sin bloqueos geográficos
        client = BinanceClient(testnet=True)
...
class BotHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/check-binance':
            # Debug endpoint - verifica datos reales de Binance TESTNET
            try:
                symbol = 'HYPEUSDT'
                interval = '3m'
                ...
                if not klines:
                    self.send_json({'error': 'No data from Binance TESTNET', 'symbol': symbol}, 500)
                    return
                ...
                self.send_json({
                    'symbol': symbol,
                    'interval': interval,
                    'environment': '🧪 TESTNET (sin bloqueos geográficos)',
                    'current_candle': {
                        'open': open_p,
                        'close': close,
                        'high': high,
                    ...
                    'status': '✅ Binance TESTNET conectado y leyendo datos',
                    'bot_strategy': bot_state['strategy']
                })
            except Exception as e:
                self.send_json({'error': str(e), 'status': '❌ Error conectando a Binance TESTNET'}, 500)
            return
