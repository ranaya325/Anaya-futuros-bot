...
def ema_cross_signal(candles, params=None):
    """EMA Cross + RSI - MÁS AGRESIVO"""
    if params is None:
        params = {}
    
    try:
        ef = int(params.get('eF', 5))      # EMA Fast - más rápida
        es = int(params.get('eS', 13))     # EMA Slow - menos diferencia
        rp = int(params.get('rP', 14))     # RSI period
        rob = int(params.get('rOB', 60))   # RSI Overbought - más sensible
        ros = int(params.get('rOS', 40))   # RSI Oversold - más sensible
        ...
        # EMAs previas
        ema_f_prev = ema(closes[:-1], ef) if len(closes) > ef else None
        ema_s_prev = ema(closes[:-1], es) if len(closes) > es else None
        
        if ema_f is None or ema_s is None or ema_f_prev is None or ema_s_prev is None:
            return None
        
        # RSI
        rs_val = rsi(candles, rp)[-1]
        if rs_val is None:
            return None
        
        # LONG: EMA Fast cruza arriba de EMA Slow + RSI no overbought
        if ema_f_prev <= ema_s_prev and ema_f > ema_s and rs_val < rob:
            return 'long'
        
        # SHORT: EMA Fast cruza abajo de EMA Slow + RSI no oversold
        if ema_f_prev >= ema_s_prev and ema_f < ema_s and rs_val > ros:
            return 'short'
        
        return None
    except Exception as e:
        add_log(f"⚠️ EMA Cross error: {e}", 'warn')
        return None
...
def bot_tick():
    ...
    try:
        ...
        # Get signal
        sig = None
        if bot_state['strategy'] == 'ema_cross':
