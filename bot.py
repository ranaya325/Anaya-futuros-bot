...
def bot_tick():
    ...
    try:
        ...
        # ── SIGNAL MODE: send Telegram alert instead of placing order ──
        ...
        msg = (
            ...
            f"R:R 1:{tp_pct/sl_pct:.1f}\n"
            f"⚠️ Esta es una señal informativa. Entra manualmente."
        )
        add_log(f"📡 Señal: {sig.upper()} @ ${price:.4f} | SL:${sl_price:.4f} TP:${tp_price:.4f}", 'trade')
        send_telegram(msg)
        bot_state['trades_today'] += 1
        bot_state['status'] = f"Última señal: {'LONG' if sig=='long' else 'SHORT'} @ ${price:.3f}"
        ge_log = f"✅ Señal enviada a Telegram: {sig.upper()} @ ${price:.4f}"
        add_log(ge_log, 'trade')
